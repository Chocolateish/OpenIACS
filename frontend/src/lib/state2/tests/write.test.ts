...
import type { StateOwnerOk, StateSet, StateSetSync } from "../types";
import {
...
let gen_sync_states = (
  setter?:
    | StateSetSync<number, StateOwnerOk<Result<number, string>>, any>
    | true
): StateTestsWrite[] => {
  return [
    ...normals(setter),
...
let gen_states = (
  setter?: StateSet<number, StateOwnerOk<Result<number, string>>, any> | true
): StateTestsWrite[] => {
  return [
    ...delayed(setter),
