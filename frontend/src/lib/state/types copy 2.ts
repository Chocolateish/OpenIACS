...
export interface STATE_XXX_WS<RT, WT = RT, REL extends STATE_RELATED = {}>
  extends STATE_BASE<RT, WT, REL, ResultOk<RT>> {
  readonly writable: true;
  readonly wsync: true;
  write(value: WT): Promise<Result<void, string>>;
...
type TEST<RT, WT = RT, REL extends STATE_RELATED = {}> =
  | STATE_REA<RT, REL>
  | STATE_ROA<RT, REL>
  | STATE_RES<RT, REL>
  | STATE_ROS<RT, REL>
  | STATE_XXX_WA<RT, WT, REL>
  | STATE_XXX_WS<RT, WT, REL>;

async function func(s: TEST<number>) {
  if (s.rsync && s.rok && s.writable && s.wsync) {
    let yo = await s;
    let yo2 = s.get();
    let yo3 = s.getOk();
    s.write(5);
