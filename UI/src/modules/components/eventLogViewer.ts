...
import "./eventLogViewer.scss";

/**Defines the event log data*/
...
/**Defines options for event list*/
export type EventLogViewerOptions = {
  manager?: ModuleManager;
} & ContentBaseOptions;

export class EventLogViewer extends Content<EventLogViewerOptions> {
  /**Returns the name used to define the element */
  static elementName() {
    return "event-log-viewer";
  }
  ...
  private __logStorage: EventLog[] = [];
  private __newestLog = 0;
  private __amount = 0;
  private __max = 0;
  private __shown: (HTMLTableRowElement | EventLogViewerLine)[] = [];
  ...
  private __manager?: ModuleManager;
  private __firstConnect?: boolean;
  private __interval: any;
  private __initListener?: ESubSubscriber<"message", any, any>;
  private __logListener?: ESubSubscriber<"message", any, any>;
  ...
  constructor() {
    ...
    this.__headers.appendChild(document.createElement("td")).innerHTML = "Text";
          ...
          if (data > 0) {
            ...
            if (this.__logStorage.length < data) {
              ...
              while (
                ...
                this.__manager!.sendMessage("LL", { type: "E", log: order });
                await new Promise((a) => {
                  setTimeout(a, 250);
                });
              }
  ...
  /**Options toggeler*/
  options(options: EventLogViewerOptions): this {
  ...
  set manager(man: ModuleManager | undefined) {
    ...
    this.__initListener = this.__manager.events.on(
      "message",
      (e) => {
        if (e.data.data["type"] === "E") {
          let oldNewestLog = this.__newestLog;
          this.__newestLog = e.data.data["newest"];
          this.__amount = e.data.data["amount"];
          this.__max = e.data.data["max"];
    ...
    this.__logListener = this.__manager.events.on(
      ...
      (e) => {
        if (e.data.data["type"] === "E")
          this.__newLog(
            this.__newestLog - e.data.data["num"] - 1,
            e.data.data["log"]
          );
      },
      ["L", "L"]
    ...
    this.__manager.sendMessage("LA", { type: "E" });
    this.__interval = setInterval(() => {
      this.__manager!.sendMessage("LA", { type: "E" });
    }, 2000);
  }

  __newLog(num: number, log: EventLog) {
    ...
    if (num >= this.__shownPosition && num < this.__shownPosition + maxShown) {
      ...
      let newLog = new EventLogViewerLine({ log: log });
  ...
  __redrawShown() {
    let orders: number[] = [];
    for (let i = 0; i < maxShown; i++) {
      let newLog: HTMLTableRowElement | EventLogViewerLine;
      let relativeNum = this.__shownPosition + i;
      if (this.__logStorage[relativeNum]) {
        newLog = new EventLogViewerLine({
    ...
    if (orders.length > 0) {
      this.__manager!.sendMessage("LL", { type: "E", log: orders });
    }
  }
...
defineElement(EventLogViewer);

/**Defines options for event list*/
type EventLogViewerLineOptions = {
  log: EventLog;
};

export class EventLogViewerLine extends WebComponent {
  /**Returns the name used to define the element */
  static elementName() {
    return "event-log-viewer-row";
  }
  static elementNameSpace() {
    return "lmui";
  }
  ...
  constructor(options: EventLogViewerLineOptions) {
    super();
    ...
    this.appendChild(document.createElement("td")).innerHTML = options.log.text;
  }
}
defineElement(EventLogViewerLine);
